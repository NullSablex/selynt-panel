...
if [ -n "$CGI_USER" ]; then
    printf "%s\n" "$CGI_USER" > "$PLUGIN_DIR/etc/da_cgi_user" 2>/dev/null || true
...
find "$PLUGIN_DIR/hooks" -type f -exec chmod 755 {} \; 2>/dev/null || true
...
if [ -d /etc/systemd/system ] && command -v systemctl >/dev/null 2>&1 && [ -f "$SERVICE_SRC" ]; then
    ...
    cp -f "$SERVICE_SRC" "$SERVICE_DST" 2>/dev/null || true
