...
### Corrigido
- **Seletor de host vazio na criação de aplicação** — a extração do JavaScript para módulos deixou para trás o bloco que preenche o `<select>` com os domínios da conta. O módulo recebia os dados e não fazia nada com eles, e o HTML só traz a opção vazia: não havia o que escolher.
- **Versões do Node fora de ordem** — o endpoint lê direto o `etc/node_versions` e devolvia na ordem gravada, que é a da varredura: no servidor saía v25, v20, v22, v24. A ordenação passa a acontecer também na leitura, então vale para arquivo gravado antes da correção.
- **Isolamento indisponível deixava o comando rodar sem ele** — o envoltório do sandbox devolvia o comando sem confinamento quando o `bwrap` faltava. Agora recusa, nomeando a causa: um pacote de terceiros rodando fora do sandbox por causa de arquivo ausente é exatamente o que ele existe para evitar.
- **Diálogos herdavam a caixa de quem veio antes** — o modal é reaproveitado entre aberturas, e a classe do anterior não era devolvida ao padrão.
...
- **CI falharia neste push** — a verificação de assets exigia `panel.min.js` e `script.min.js`, que deixaram de existir quando os módulos compartilhados foram separados. Além disso, o lint de PHP filtrava por `.php`, `.html` e `.raw`, e as páginas de `user/` e `admin/` perderam a extensão quando as URLs deixaram de expô-la: justamente os maiores arquivos PHP saíram da verificação. De 19 para 27 arquivos conferidos.
- **Pacote de instalação levava o repositório inteiro** — o tarball do release incluía o `.git` (1172 objetos, 8,9 MB de histórico), as notas de changelog do DirectAdmin, os requisitos do mkdocs e a ferramenta de teste dos módulos. A etapa que deveria barrar isso não pegava nada: a lista de proibidos falava em `.github` sem barra, que não casa com `.git/`. O pacote cai de 7,7 MB para 2,4 MB.
- **Release falharia ao publicar esta versão** — a verificação do pacote exigia `panel.min.js` e `script.min.js`, que deixaram de existir quando os módulos compartilhados foram separados por responsabilidade. Passa a exigir os dezenove arquivos que o painel carrega hoje.
...
### Alterado
- **Actions do GitHub fixadas por SHA** — uma tag é móvel: quem controla o repositório da action pode reapontar `@v7` para outro commit, e o workflow roda o que vier, com o token do repositório em mãos. O token também deixa de nascer com poder de escrita — `release.yml` declarava `contents: write` no topo, valendo para todos os jobs, e a permissão desce para o job que publica.
- **Labels do repositório sincronizados por workflow** — o `labels.yml` existia e nada o aplicava, então os labels que o labeler cria vinham sem cor nem descrição. O labeler também aplicava `docs`, que não estava definido, e casava `user/**/*.html` — glob que não pega mais nada desde que as páginas perderam a extensão.
...
### Adicionado
- **Executar comandos npm pelo painel** — `Instalar`, `Atualizar` e os scripts declarados no `package.json`, sem SSH. Quem usa hospedagem compartilhada em geral não tem terminal, e sem isso não conseguia nem instalar as dependências da própria aplicação. Cada execução roda confinada ao diretório da aplicação: um `postinstall` comprometido não alcança o `.env` do app vizinho. O comando roda no servidor, desprendido da requisição — fechar a página não o interrompe, e reabrir reencontra a execução onde ela está.
- **Painel lateral de execuções** — no canto da tela, sobre a página e sem escurecer o fundo: acompanhar um comando não impede de usar o painel. Sem execuções é só um ícone; com jobs, lista o que roda e o que terminou, e nada some sozinho — um job encerrado é registro de algo que aconteceu, e sai quando o usuário limpar.
- **Estado das dependências na tela do app** — distingue cinco situações e oferece só a ação que faz sentido: oferecer "Instalar" e "Atualizar" com tudo em ordem contradiz o próprio aviso. Sem `node_modules`, o painel diz que a aplicação não vai iniciar, em vez de deixar o cliente descobrir pelo log de erro.
- **`package.json` criado junto com a aplicação Node** — antes ela nascia só com o arquivo de entrada, e o cliente precisava criá-lo por FTP para usar qualquer coisa de npm. É o `npm init` de verdade: o painel não escolhe entre CommonJS e ESM, nem entre JavaScript e TypeScript.
- **Arquivo de entrada editável** — era decidido na criação e ficava assim para sempre; quem migrasse de `index.js` para `app.mjs` só resolvia apagando a aplicação e criando outra.
- **Documentos de contribuição** — `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `AI-POLICY.md`, templates de issue e de pull request. O uso de IA é permitido e não sofre preconceito; quem contribui responde pelo que envia, e não se atribui co-autoria a um assistente. Nenhum deles vai no pacote de instalação.
- **`npm run check`, que executa os módulos** — cada módulo do painel roda num DOM mínimo, para pegar o que o código HTTP não mostra: servir com 200 diz que o arquivo chegou, não que ele roda. Foi assim que apareceram as funções perdidas na extração do JavaScript. Depois de executar, confere também se os handlers citados nos `onclick` chegaram ao `window`: a função pode existir e rodar sem erro, e o botão ainda assim não fazer nada, porque o atributo é resolvido no escopo global e o módulo tem o seu próprio.
